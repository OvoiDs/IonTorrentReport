...
__version__ = 1.3
__date__ = '2014-03-05'
__updated__ = '2014-27-06'

SNPSIFT = "/data/galaxy/software/snpEff-4.2/SnpSift.jar"
SNPEFF = "/data/galaxy/software/snpEff-4.2/snpEff.jar"
DBSNP = "/data/galaxy/genome_db/dbsnp_138.hg19.vcf"
SNPEFF_CONFIG = "/data/galaxy/software/snpEff-4.2/snpEff.config"
DBNSFP = "/data/galaxy/genome_db/dbNSFP2.9.txt.gz"
TABIX = "/data/galaxy/software/tabix-0.2.6/tabix"
REFSEQPOSITIONS = "/data/galaxy/genome_db/refGene_15062014.sorted.txt.gz"
COSMIC = "/data/galaxy/genome_db/CosmicCodingMuts_v69_05062014.vcf.gz"
CANCERPANEL = "/data/galaxy/genome_db/SAFIR/CancerPanel.tsv"
...
class OutputXls(object):
    def __init__(self,outputfile, cancerpanel):
...
class Annotation(object):
    ...
    def rank(self, annotation, prioritize):
        ...
        if (self.type == "exonseq"):
            ...
            High = re.compile(r"splice|initiator_codon_variant|stop_retained_variant|protein_protein_contact")
...
class VCF():
    '''All of this is derived from another program. Please forgive me for the crapiness. Feel free to reuse, or even better, to make that darn module I never had time to do.'''
...
''' % (program_version_message, program_shortdesc, str(__date__))
    ...
    try:
        ...
        parser.add_argument("-o", "--output", dest="output", help="XLS output filename", required=True, type=str)
        parser.add_argument("-t", "--tabular", dest="tab", help="TSV output filename", required=True, type=str)
        parser.add_argument("-n", "--name", dest="name", required=True, type=str, help="This will be included in the first column of tab and xls files")
    ...
    #Annotation
    SnpSift1 = ["java", "-jar", "-Xmx2000m", SNPSIFT, "annotate", "-v", DBSNP, vcf_input]
    SnpEff1 = ["java", "-jar", "-Xmx2000m", SNPEFF, "eff", "-c", SNPEFF_CONFIG, "-v","-formatEff", "-t", "-noStats", "-noMotif", "-noNextProt", "hg19", filename1]
    SnpSift2 = ["java", "-jar", "-Xmx2000m", SNPSIFT, "dbnsfp", "-v", "-db", DBNSFP, filename2]
    print " ".join(SnpSift1)
    print " ".join(SnpEff1)
    print " ".join(SnpSift2)
    ...
    book = OutputXls(output, cancer_panel)
    book.column_headers()
